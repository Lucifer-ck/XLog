...
// create dump_info from ranges
template<typename It>
inline details::dump_info<It> to_hexEx(It sPointer,It ePointer, size_t size_per_line = 32)
{
    std::vector<It> v;
    for (int i = 0; i < ePointer - sPointer; i++) v.push_back(sPointer+i);
    return details::dump_info<It>(v.begin(), v.end(), size_per_line);
}
} // namespace spdlog
